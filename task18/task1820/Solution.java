...
public class Solution {
    ...
    public static Optional<Car> getMostExpensiveCar(Stream<Car> cars) {
       // Optional<Car> findCar = Optional.of(cars.max((model1, model2) -> model1.getPrice() - model2.getPrice()).get());
    ...
    public static Optional<Car> getMoreExpensiveCar(Stream<Car> cars, Car mostExpensiveCar) {
        Optional<Car> findCar = cars.filter(model1 -> model1.getPrice() > mostExpensiveCar.getPrice()).findFirst();
